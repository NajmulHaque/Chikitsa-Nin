...
<div class="container">
    <div class="row">
        <div class="col-md-4">
            <form action="{{ route('requestmedical') }}" method="get">
                <select name="category" id="map-option">
                    <option value="">Category</option>
                    @foreach ($categories as $category)
                    <option value="{{$category->id}}"{{ old('category', request()->input('category')) == $category->id ? ' selected' : '' }}>
                        {{ $category->name}}
                    </option>
                    @endforeach
                </select>
                                        ...
                                        <li>
                                            <div class="geodir-post-title">
                                                <h4 class="geodir-entry-title">
                                                    <a href="{{ route('requestmedical.index', '') }}/`+place.id+`">`+place.name+`</a>
                                                </h4>
                                            </div>
